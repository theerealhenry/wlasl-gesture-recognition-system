...
_BOOTSTRAP_SIGNER_CAVEAT: str = (
    "Resampling unit is the individual clip, not the signer. Validation "
    "clips cluster by signer (e.g. 52 clips across 7 signers) and "
    "prediction errors are plausibly correlated within a signer. This "
    "bootstrap therefore likely UNDERSTATES true uncertainty relative to a "
    "signer-stratified block bootstrap, because the effective sample size "
    "is probably smaller than the clip count suggests. Treat this interval "
    "as a lower bound on uncertainty, not a tight estimate."
)
